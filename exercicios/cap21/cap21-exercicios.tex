...
Para saber quantas funções bijetoras poderemos formar, considere a figura \ref{cap21-exercicios-C1-pag160-2}
   ...
   \begin{itemize}
     \item[\ding{172}] \textbf{O que contar?} Tratam-se de 4 experimentos. Para cada elemento do domínio $\{1,2,3,4\}$ , deve-se escolher, dentre os elementos disponíveis do contradomínio $\{5,6,7,8\}$, outro elemento de forma que a função seja bijetora (restrição).
     \item[\ding{173}] \textbf{Restrição:} Para cada elemento do domínio $\{1,2,3,4\}$ , deve-se escolher, dentre os elementos disponíveis do contradomínio $\{5,6,7,8\}$, outro elemento de forma que a função seja bijetora (restrição).
     \item[\ding{174}] \textbf{Experimento 1:} $E_1$ = Para o primeiro elemento do domínio "1" em $A$, devemos escolher um elemento do contradomínio em $B$ de forma a respeitar a restrição do enununciado da questão.
   \end{itemize}  
